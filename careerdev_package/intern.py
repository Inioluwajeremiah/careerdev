...
def getInternPage(id):
    ...
    related_posts = PostModel.query.filter_by(post_cat="Internship").filter(PostModel.id.in_(ids)).all()

    return render_template('internDetails.html', singlePost=singlePost, user=current_user, related_posts=related_posts)
